...
class Book(models.Model):
    ...
    rate = models.PositiveSmallIntegerField(blank=True, default=0, help_text='Ocena użytkownika (min=0, max=5)',
                                            validators=[MinValueValidator(0), MaxValueValidator(5)])
    status = models.PositiveSmallIntegerField(blank=True, null=True, choices=Status.choices,
                                              help_text='Status książki: 0-do przeczytania, 1-w trakcie, 2-przeczytana')
    ...
    created_by = models.ForeignKey(User, on_delete=models.CASCADE, editable=False,
                                   help_text='Użytkownik, który utworzył pozycję')
